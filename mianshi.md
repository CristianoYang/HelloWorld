...
12. 有哪些线程池？
    ...
    - newCachedThreadPool,是一种线程数量不定的线程池，并且其最大线程数为Integer.MAX_VALUE，这个数是很大的，一个可缓存线程池，如果线程池长度超过处理需要，可灵活回收空闲线程，若无可回收，则新建线程。但是线程池中的空闲线程都有超时限制，这个超时时长是60秒，超过60秒闲置线程就会被回收。
...
17. 解释一下 CountDownLatch 概念？给出一些 CountDownLatch 使用的例子？不足之处？
...
11. JVM调优
    ...
    对jvm调优主要是为了减少GC频率和fullGC的次数。
    
    （1）新生代GC（MinorGC）：eden、survivor from、survivor to默认比例8:1:1；
    
    新产生的对象优先进去Eden区，当Eden区满了之后再使用Survivor from，当Survivor from 也满了之后就进行Minor GC（新生代GC），将Eden和Survivor from中存活的对象copy进入Survivor to，然后清空Eden和Survivor from，这个时候原来的Survivor from成了新的Survivor to，原来的Survivor to成了新的Survivor from。复制的时候，如果Survivor to 无法容纳全部存活的对象，则根据老年代的分配担保（老年代进行担保）（类似于银行的贷款担保）将对象copy进去老年代，如果老年代也无法容纳，则进行Full GC（老年代GC）。
    
    （2）老年代GC（fullGC）：
    
    大对象直接进入老年代：JVM中有个参数配置-XX:PretenureSizeThreshold，令大于这个设置值的对象直接进入老年代，目的是为了避免在Eden和Survivor区之间发生大量的内存复制。
    
    如果对象在Eden出生并经过第一次Minor GC后仍然存活，并且能被Survivor容纳，将被移入Survivor并且年龄设定为1。没熬过一次Minor GC，年龄就加1，当他的年龄到一定程度（默认为15岁，可以通过XX:MaxTenuringThreshold来设定），就会移入老年代。
...
​	java实现反射机制主要借助4个类：class、Constructor、Field、Method

- class：常量池中存放了类的几乎所有信息，类名、字段名、方法名、符号引用等
- Constructor：几乎可以获得类的所有构造方法
- Field：获得类声明的所有字段
- Method：获得类声明的所有方法
...
10. cloneable接口实现原理？
    ...
    这个方法中有一个native关键字修饰，判断是否实现cloneable接口，是在调用jvm中的实现体时进行判断的

11. java异常机制

    java中的异常类均以Throwable为父类，而Throwable又派生出了两个类Error和Exception。

    （1）Error：程序中无法处理的错误，表示运行应用程序中出现了严重的错误。

    - Virtual MachineError（虚拟机运行错误），StackOverFlowError，OutOfMemoryError
    - NoClassDefFoundError（类定义错误）

    （2）Exception：程序本身可以捕获并且可以处理的异常。

    - RuntimeException运行时异常（非检查型）：NullPointerException空指针异常、ClassNotFoundException
    - checkedException检查型异常，IOException，发生是可以预测的

    throw和throws区别：

    - throw在方法内部，抛出异常对象：throw new 异常
    - throws运用于方法声明之上，用于表示当前方法不处理异常，而是提醒该方法的调用者来处理异常：throws 异常类名1，异常类名2 ... { }

12. public、private、default、protected区别？

    public：类内部、同一个包内、子类、其他都可以访问

    protected：类内部、同一个包下、子类都可以访问

    default：类内部、同一个包下都可以访问

    private：类内部可以访问
    
13. servlet的执行原理和生命周期？

    生命周期：servlet加载、实例化、服务、销毁

    - init（）初始化，仅执行一次，服务器装入Servlet时执行。
    - service（)核心方法，负责相应客户请求。每当一个客户请求一个HttpServlet对象，该对象的Service()方法就要调用，而且传递给这个方法一个“请求”（ServletRequest）对象和一个“响应”（ServletResponse）对象作为参数。
...
13. 怎么查看SQL执行过程？如何对SQL语句优化？explain参数？
...
1. 五层协议体系结构？七层体系结构？为什么要分层？
   ...
   - 独立性强。独立性是指对具有相对独立功能的每一层，它不必知道下一层是如何实现的，只要知道下层通过层间接口提供的服务是什么，本层向上一层提供的服务是什么就可以了。
   - 功能简单。系统经分层后，整个复杂的系统被分解成若干个范围小的、功能简单的部分，使每一层功能简单，进行程序设计和实现比较方便。
   - 适应性强。当任何一层发生变化，只要层间接口不发生变化，那么这种变化就不会影响其他任何一层，这就意味着可以对分层结构中的任何一层的内部进行修改而不影响其他层。
   - 易于实现和维护。分层结构使实现和调试一个庞大而复杂的网络系统变得简单和容易。
   - 方便标准化
...
3. 半打开状态和半关闭状态？
   ...
   **半关闭：**由于在全关闭的状态下，当服务器端的数据发送完毕要关闭连接的时候，这时客户端会接收到服务器端的请求，但由于使用全关闭状态，则客户端向服务器端发送的断开连接确认请求将无法再次返回给服务器端。此时就需要用到半关闭状态。为了让数据之间完全交换，也为了让客户端和服务器端之间的数据得到可控，而防止阻塞的情况发生，因此应该用shutdown函数在服务器端发送完数据之后，应该附带一个EOF，让客户端知道服务器端的数据已经发送完成。
...
20. tcp报文
    ...
    ![点击查看源网页](https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fseo-1255598498.file.myqcloud.com%2Ffull%2Fa509d71b92140777c21405e818f2a5d34c47b6ba.jpg&refer=http%3A%2F%2Fseo-1255598498.file.myqcloud.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1619587866&t=42517520d6ac6ff331c83aafdda979f2)
...
16. 孤儿进程和僵尸进程？怎么避免产生僵尸进程？

    孤儿进程：父进程结束了，而它的一个或多个子进程还在运行，那么这些子进程就成为孤儿进程(father died)。子进程的资源由init进程(进程号PID = 1)回收。因为会被1号进程回收，所以孤儿进程不会有什么危害。

    僵尸进程：子进程退出了，但是父进程没有用wait或waitpid去获取子进程的状态信息，那么子进程的进程描述符仍然保存在系统中，这种进程称为僵死进程。一般，为了防止产生僵尸进程，在fork子进程之后我们都要wait它们；同时，当子进程退出的时候，内核都会给父进程一个SIGCHLD信号，所以我们可以建立一个捕获SIGCHLD信号的信号处理函数，在函数体中调用wait（或waitpid），就可以清理退出的子进程以达到防止僵尸进程的目的。
...
1. zookeeper详解
   ...
   整体上可以看做一棵树，每个节点称做ZNode，默认存储1M的数据。

   znode可以分为两种类型：

   - 持久性节点：客户端和服务器断开连接后，创建的节点保留
   - 临时性节点：客户端和服务器断开连接后，创建的节点自己删除
   ...
   半数机制：集群中半数以上机器存活，集群可用。存在leader节点和follower节点

   没有leader节点时，服务器都属于looking状态，以（sid，zxid）进行投票，优先检查zxid。zxid比较大的服务器优先作为leader。如果zxid相同的话，就比较sid，sid比较大的服务器作为leader。

   （3）工作原理

   - 核心就是原子广播。在ZooKeeper中所有的事务请求都由一个主服务器也就是Leader来处理，其他服务器为Follower，Leader将客户端的事务请求转换为事务Proposal，并且将Proposal分发给集群中其他所有的Follower，然后Leader等待Follwer反馈，当有  过半数（>=N/2+1）   的Follower反馈信息后，Leader将再次向集群内Follower广播Commit信息，Commit为将之前的Proposal提交。

   - zap协议两种实现方式
     - **恢复模式：当服务启动或在leader崩溃后，ZAB就进入了恢复模式，在leader选举出来之前，且大多数server完成和leader的状态同步以后，恢复模式就结束了。在这个过程中，要保证被leader提交的proposal最终被所有的follower执行，确保那些只在leader提出的proposal被丢弃。**
     - **广播模式：即leader提出一个决议，由follower进行投票，leader对投票结果进行计算决定是否通过决议，如果通过执行该决议，否则什么都不做。**
...
2. 如何保证消息队列的高可用性？
   ...
   - 普通集群：一个queue保存实例，其他的节点通过该实例的queue获取数据

     没做到真正的高可用，存在数据拉取和单实例的瓶颈问题。

   - 镜像集群，每个queue都保存实例，有效率问题。
...
你好，我叫杨建，目前研二，就读于南京理工大学软件工程专业。
本科就读于湖南大学。
技术方向为Java后台开发，熟悉java基础\java容器、jvm虚拟机、并发控制、反射机制等；
数据库熟悉mysql，包括索引、事务等，redis数据库，了解数据结构、持久化、主从复制等，了解计算机网络和操作系统基础。
项目方面，实现过开源商城、开原博客，技术栈包括springboot、Thmyleam、mybatis等。
